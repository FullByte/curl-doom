...
static const char* weapon_name(weapontype_t weapon) {
    switch (weapon) {
        ...
        case wp_nochange: return "nochange";
...
static void dump_state(void) {
    ...
    if (p == NULL || p->mo == NULL) {
        fprintf(stderr, "STAT {\"ok\":false}\n");
        fflush(stderr);
        return;
    }

    const double x = (double)p->mo->x / 65536.0;
    const double y = (double)p->mo->y / 65536.0;
    const double z = (double)p->mo->z / 65536.0;
    const double secret_pct = (totalsecret > 0)
        ? (100.0 * (double)p->secretcount / (double)totalsecret)
        : 0.0;

    fprintf(stderr,
            "STAT {\"ok\":true,\"episode\":%d,\"map\":%d,\"leveltime\":%d,"
            "\"x\":%.4f,\"y\":%.4f,\"z\":%.4f,\"angle\":%u,"
            "\"health\":%d,\"armor\":%d,\"secret_count\":%d,\"secret_total\":%d,\"secret_pct\":%.1f,"
            "\"weapon_id\":%d,\"weapon\":\"%s\","
            "\"key_blue_card\":%d,\"key_yellow_card\":%d,\"key_red_card\":%d,"
            "\"key_blue_skull\":%d,\"key_yellow_skull\":%d,\"key_red_skull\":%d,"
            "\"ammo_clip\":%d,\"ammo_shell\":%d,\"ammo_cell\":%d,\"ammo_misl\":%d}\n",
            gameepisode,
            gamemap,
            leveltime,
            x,
            ...
            secret_pct,
            (int)p->readyweapon,
            weapon_name(p->readyweapon),
            p->cards[it_bluecard] ? 1 : 0,
            p->cards[it_yellowcard] ? 1 : 0,
            p->cards[it_redcard] ? 1 : 0,
            p->cards[it_blueskull] ? 1 : 0,
            p->cards[it_yellowskull] ? 1 : 0,
            p->cards[it_redskull] ? 1 : 0,
            p->ammo[am_clip],
            p->ammo[am_shell],
            p->ammo[am_cell],
            p->ammo[am_misl]);
